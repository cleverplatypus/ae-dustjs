(function (root, factory) {
  ...
  var solve = function(inBase, inSeg) {
    if(dust.propertyResolver) {
      return dust.propertyResolver(inBase, inSeg);
    } else {
      return inBase[inSeg];
    }
  }
    ...
    if (cur && len === 0) {
      ...
      if (!cur) {
        ...
        if (value !== undefined) {
          ...
          ctx = this.global && solve(this.global, first);
        }
      } else if (ctx) {
        // if scope is limited by a leading dot, don't search up the tree
        if(ctx.head) {
          ctx = solve(ctx.head,first);
        } else {
  ...
  Chunk.prototype.section = function(elem, context, bodies, params) {
    ...
    */
    if(dust.collectionResolver) {
      elem = dust.collectionResolver(elem);
    }
